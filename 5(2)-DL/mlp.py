...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h4 = z3 @ W4 + b4#TODO
        y_hat = sigmoid(h4)#TODO
        y_hat = y_hat.reshape(X.shape[0],)#TODO
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dh3=dz3*(1-z3 ** 2)  #tanh
        db3=np.sum(dh3,axis=0) #역전파 할때 각 데이터들 gradient 더함
        dW3= z2.T @ dh3 +2*L2_norm * W3
        dz2= dh3 @ W3.T #다음에 넘겨줄꺼
        ...
        dh2=dz2* np.where(h2>0,1,0.01) #leakyrelu
        db2= np.sum(dh2,axis=0)
        dW2= z1.T @ dh2 + 2*L2_norm*W2
        dz1= dh2 @ W2.T # 다음에 넘겨줄 꺼
